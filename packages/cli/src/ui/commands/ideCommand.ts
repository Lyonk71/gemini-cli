...
export const ideCommand: SlashCommand = {
  ...
  action: async (
    ...
    if (action === 'install' || action === 'install-extension') {
      const currentIde = context.services.config?.getIdeClient().getCurrentIde();
      ...
      if (!currentIde) {
        return {
          type: 'message',
          messageType: 'error',
          content: 'No IDE detected. Please ensure VSCode or JetBrains IDE is running.',
        };
      }
    ...
    if (action === 'connect') {
      ...
      try {
        ...
        const statusMessage = getIdeStatusMessage(ideClient);
        return {
          type: 'message',
          messageType: statusMessage.messageType,
          content: statusMessage.content,
